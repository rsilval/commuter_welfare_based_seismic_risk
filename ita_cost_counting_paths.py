...
class ITA:
  def __init__(self, G, demand,counting,idx):
    #G is a networkx graph, it can be damaged. Edges need to have a free flow travel time, a capacity, a variable called flow (which we'll change to keep track of flows assigned to each link), and a variable called t_a (which we'll change based on the flows)
    self.G = G
    self.demand = demand
  ...
  def assign(self):
    ...
    if idx == 1:
      for origin in self.demand:
        for destination in self.demand[origin]:
          time_value = 0
          paths_list_od = path_dict[origin][destination][1]
          for i in range(0,len(paths_list_od)-1):
              time_value += edge_times[(paths_list_od[i], paths_list_od[i + 1])]
              print edge_times[(paths_list_od[i], paths_list_od[i + 1])]
              #print edge_times[(paths_list_od[i + 1], paths_list_od[i])]
